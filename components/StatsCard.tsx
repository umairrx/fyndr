...
export default function StatsCard({
  ...
  const isNumeric =
    Boolean(match) && !isNaN(parseFloat(match![2].replace(/,/g, "")));
  ...
  return (
    <div
      className={`group px-6 md:px-12 py-4 md:py-6 rounded-2xl md:rounded-4xl shadow-md 
    ...
    >
      <div className={`pb-2 ${tag ? "flex justify-between items-center" : ""}`}>
        <p
          className={`text-2xl md:text-4xl font-bold ${isDark ? "text-primary-color group-hover:text-black" : "text-custom-gray-dark"} 
        ...
        {tag && (
          <div
            className={`${isDark ? "text-black border border-black" : "bg-black text-white border border-black"} 
      ...
      <div className="flex justify-between items-center">
        <p
          className={`text-sm md:text-base ${isDark ? "text-gray-100 group-hover:text-black" : "text-custom-gray-dark"}`}
        >
          {description}
        </p>
        {!tag && !isDark && (
          <div
            className="bg-black text-white p-1.5 md:p-2 rounded-full ml-2
